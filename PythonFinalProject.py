...
def model_restore_from_pb(pb_path,node_dict):
    config = tf.compat.v1.ConfigProto(log_device_placement=True,#印出目前的運算是使用CPU或GPU
...
def video_init(is_2_write=False,save_path=None):
    writer = None
    cap = cv2.VideoCapture(0)#//0是用自己的筆電攝影機 而1是usb攝像頭
...
def mask_detection(is_2_write=False,save_path=None):
    #----var C:\Users\user\Desktop\opencvtest
    pb_path = r"C:\Users\user\Desktop\opencvtest\face_mask_detection.pb"
    ...
    while (cap.isOpened()):
        ...
        if ret:
            ...
            for idx in keep_idxs:
                ...
                y.append(time.strftime("%H:%M:%S"))
                print(x)
                print(y)
                print(conf)
                print(id2class[class_id])
                '''
                connection = pymysql.connect(host='localhost',#從265(這一行)到278行都是上傳到資料庫
                    user='root',
                    port=3306,
                    password='',
                    db='nodemcu_rfidrc522_mysql',
                    cursorclass=pymysql.cursors.DictCursor)
                try:
                    with connection.cursor() as cursor:
                        print('hi')
                        sqlQuery =  "INSERT INTO mask_status(`mask`) VALUES (%s)"  
                        cursor.execute(sqlQuery, (id2class[class_id]))
                        connection.commit()
                finally:
            ...
            #----'q' key pressed?
                ...
                try:
                    with connection.cursor() as cursor:
                        #sqlQuery =  "INSERT INTO sdata(`Date`, `Open`, `High`, `Low`, `Close`, `Volume`) VALUES (%s, %s, %s, %s, %s, %s)"
                        #cursor.execute(sqlQuery,(time.strftime("%H"),time.strftime("%M"),time.strftime("%S"), float(53.21), float(98.56), int(9800)))
                        sqlQuery =  "INSERT INTO tii(`time`,`mask`) VALUES (%s, %s)"
                        
                        cursor.execute(sqlQuery,(time.strftime("%c"), id2class[class_id]))
                        connection.commit()
                                '''
                ...
                #        #sqlQuery =  "INSERT INTO sdata(`Date`, `Open`, `High`, `Low`, `Close`, `Volume`) VALUES (%s, %s, %s, %s, %s, %s)"
                #        #cursor.execute(sqlQuery,(time.strftime("%H"),time.strftime("%M"),time.strftime("%S"), float(53.21), float(98.56), int(9800)))
                #        sqlQuery =  "INSERT INTO mtest(`hour`, `minute`, `second`, `mask`) VALUES (%s, %s, %s, %s)"
                #        cursor.execute(sqlQuery,(time.strftime("%H"),time.strftime("%M"),time.strftime("%S"), id2class[class_id]))
                #        connection.commit()
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                ...
                print(x)
                print(y)
                plt.plot(y,x,label="hi")
                plt.title('graph')
                plt.ylabel('chanceOfWearingMask')
                plt.xlabel('times')
                plt.show()
                break
        else:
